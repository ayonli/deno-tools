...
import { DenoDocumentFormattingEditProvider } from '../formatter';
    ...
    suite('Language ID Fallback Logic', () => {
        test('Should map language IDs correctly when extension detection fails', () => {
            ...
            testCases.forEach(({ languageId, expected }) => {
                ...
                assert.strictEqual(result, expected, `Language ID ${languageId} should map to ${expected}`);
            });
        });
...
function getDenoExtensionForTest(formatter: DenoDocumentFormattingEditProvider, fileName: string, languageId: string): string {
